...
class AutentificadorMW
{
	...
	public function VerificarAcceso($request, $response, $next) {
		...
		{		
			...
			$payload=AutentificadorJWT::ObtenerData($token);
	///__descomentar para log		$nombre = $payload->nombre; 
///__descomentar para log			$mail = $payload->mail;
	///__descomentar para log		$perfil = $payload->perfil;
			...
			if($request->isPost()){
				...
				$tarea = "Imprimir listado";
		///__descomentar para log		self::ArchivarIngresos($nombre,$mail,$perfil,$tarea,$fecha,$hora);
			$response = $next($request, $response);	
